...
def test_status_transition_records_history(conn):
    db.upsert_broker(conn, BROKER)
    broker = db.all_brokers(conn)[0]
    req = db.get_or_create_request(conn, broker.id)
...
def test_sent_sets_followup_60_days_for_people_search(conn):
    db.upsert_broker(conn, BROKER)
    broker = db.all_brokers(conn)[0]
    req = db.get_or_create_request(conn, broker.id)
...
def test_sent_sets_followup_90_days_for_marketing(conn):
...
def test_due_requests_excludes_terminal_and_future(conn):
    db.upsert_broker(conn, BROKER)
    broker = db.all_brokers(conn)[0]
    req = db.get_or_create_request(conn, broker.id)
    db.set_status(conn, req.id, STATUS_SENT)
